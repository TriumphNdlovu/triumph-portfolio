    ...
    <>
      <div className="space-y-8">
        {Object.entries(groupedSkills).map(([category, skills]) => (
          <div key={category}>
            <h2 className="text-2xl text-maintextColor font-pixel font-semibold mb-4">{category}</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
              {skills.map((skill, index) => (
                ...
                >
                  <h3 className="text-lg text-secondaryTextColor font-pixel font-semibold mb-2">{skill.name}</h3>
                </div>
              ))}
            </div>
          </div>
